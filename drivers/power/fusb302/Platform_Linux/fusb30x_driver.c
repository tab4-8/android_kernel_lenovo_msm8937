                          ...
                          const struct i2c_device_id* id)
{
    ...
    dev_info(&client->dev, "%s~%d\n", __func__,tusb422_exit);
    if(tusb422_exit == true)
    {
    	pr_err("fusb302 tusb422 has probe ok, fusb302 not exit!!!\n");
	return -ENODEV;  	
    }
    /* Make sure probe was called on a compatible device */
	if (!of_match_device(fusb30x_dt_match, &client->dev))
	{
		pr_err("fusb302 FUSB  %s - Error: Device tree mismatch!\n", __func__);
		return -EINVAL;
	}
   // printk("fusb302 FUSB  %s - Device tree matched!\n", __func__);
    ...
    if (!chip)
	{
		pr_err("fusb302 FUSB  %s - Error: Unable to allocate memory for g_chip!\n", __func__);
		return -ENOMEM;
	}
   ...
   // printk("fusb302 FUSB  %s - Chip struct data initialized!\n", __func__);
    ...
    else
    {
        ...
        {
            pr_err("fusb302 FUSB  %s - Error: Required I2C/SMBus functionality not supported!\n", __func__);
            pr_err("fusb302 FUSB  %s - I2C Supported Functionality Mask: 0x%x\n", __func__, i2c_get_functionality(adapter));
            return -EIO;
        }
    }
    ...
    {
        pr_err("fusb302 FUSB  %s - Error: Unable to communicate with device!\n", __func__);
        return -EIO;
    }
    ...
    if (ret)
    {
        pr_err("fusb302 FUSB  %s - Error: Unable to initialize GPIO!\n", __func__);
        return ret;
    }
   ...
   // printk("fusb302 FUSB  %s - Sysfs device file created!\n", __func__);
    ...
    if (ret)
    {
        pr_err("fusb302 FUSB  %s - Error: Unable to enable interrupts! Error code: %d\n", __func__, ret);
        return -EIO;
    }

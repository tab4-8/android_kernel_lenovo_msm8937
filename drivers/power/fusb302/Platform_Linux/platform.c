...
extern int qpnp_typec_configure_ssmux(struct qpnp_typec_chip *chip,
...
void platform_set_data_role(FSC_BOOL PolicyIsDFP)
{
	int rc;
	struct qpnp_typec_chip *chip = tusb_typec_chip;
	pr_info("fusb302 func=%s,line=%d,PolicyIsDFP=%d~%d\n",__func__,__LINE__,PolicyIsDFP,typec_direction);	
	if (PolicyIsDFP)
       {
        // DFP/HOST
        //printk("%s: host dfp\n", __func__); 
        	rc = qpnp_typec_configure_ssmux(chip, typec_direction);
	if (rc) {
        ...
        {
	     //Device
        //pr_info("%s: TYPEC_ATTACHED_SNK devices\n", __func__);
         rc = qpnp_typec_configure_ssmux(chip, typec_direction);
	if (rc) {
		pr_err("failed to configure ss-mux rc=%d\n", rc);
		//return rc;

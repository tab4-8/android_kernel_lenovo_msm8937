...
static int  kongkou_gpio;

#define PROC_CAPSENSOR_RF_STATUS "cap_rf_channel_status"
static ssize_t capsensor_read_RF_channel_status(struct file *,char __user *,size_t,loff_t *);
static const struct file_operations config_read_rf_channel_ops = {
	.owner = THIS_MODULE,
	.read = capsensor_read_RF_channel_status,
};
...
#define PROC_CAPSENSOR_CHECK_FILE "capsensor_check"
static ssize_t capsensor_config_check_proc(struct file *, char __user *, size_t, loff_t *);
static const struct file_operations config_proc_capsensor_check_ops = {
	.owner = THIS_MODULE,
	.read = capsensor_config_check_proc,
};

#define PROC_KONGKOU_STATUS "kongkou_status"
static ssize_t kongkou_status_read_proc(struct file *,char __user *,size_t,loff_t *);
static struct proc_dir_entry *kongkou_proc = NULL;

static const struct file_operations config_kongkou_proc_ops ={
	.owner = THIS_MODULE,
	.read = kongkou_status_read_proc,
};
static void ForcetoTouched(psx93XX_t this)
{
	...
	value = gpio_get_value(kongkou_gpio);
	if (value) return;
...
static irqreturn_t handle_kongkou_irq(int irq, void *data)
{

	psx93XX_t this = (psx93XX_t)data;
	psx9310_t pDevice = NULL;
	struct input_dev *input = NULL;
	int value;
	int err = 0;

	if (!gpio_is_valid(kongkou_gpio)){	
		printk("kongkou irq_gpio was not assigned properly");
	}
	value = gpio_get_value(kongkou_gpio);
	if(value)
	{
		...
		if (sx9310_enable){
			...
			err = sx9310_power_ctl(this, false);     //modified by dingjian for power supply when system suspend 20170322
			if (err) {
		...
		//enable capsensor
		err = sx9310_power_ctl(this, true);     //modified by dingjian for power supply when system suspend 20170322
		if (err) {
			dev_err(this->pdev, "Failed to disable Capacitive Touch Controller power\n");
		}
		sx9310_enable = 1;
	...
	input = pDevice->pbuttonInformation->input;
	input_report_key(input, KEY_KONGKOU_KEYCODE , value); 
	input_sync(input);
...
//		}
		int value = gpio_get_value(kongkou_gpio);
		if(value)
		{
			psx9310_t pDevice = NULL;
			struct input_dev *input = NULL;
...
int sx93XX_init(psx93XX_t this)
{
	...
	enable_irq_wake(this->irq);   //use capsensor intrrupt as system wake up source ,modified by dingjian 20170322
	err = request_threaded_irq(this->kongkou_irq,NULL,handle_kongkou_irq,IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_ONESHOT, this->pdev->driver->name,this);
	if (err){
//		irq_free(this->irq,this);
		dev_err(this->pdev, "kongkou irq request failed %d, error %d\n", this->kongkou_irq, err);
	}
...
#define SX9310_VDD_ENABLE 1 /* [P3592] for control vdd by gpio 141 , add by djian 20170123 pm*/
#define SX9310_VDD_DISABLE 0 /* [P3592] for control vdd by gpio 141 , add by djian 20170123 pm*/
...
static int sx9310_power_ctl(sx93XX_t *data, bool on)
{
	...
	if (!on && data->power_enabled) {
		ret = gpio_direction_output(data->vdd_ctrl,SX9310_VDD_DISABLE); /* [P3592] for control vdd by gpio 141 , add by djian 20170123 pm*/
		if (ret) {
			pr_err("Gpio vdd disable failed ret=%d\n", ret);
			return ret;
		}
		...
		ret = gpio_direction_output(data->vdd_ctrl,SX9310_VDD_ENABLE); /* [P3592] for control vdd by gpio 141 , add by djian 20170123 pm*/
		if (ret) {
			pr_err("Gpio vdd enable failed ret=%d\n", ret);
			return ret;
		}
...
{
	int ret;
	struct device_node *np = data->pdev->of_node;

	data->vdd_ctrl = of_get_named_gpio(np, "Semtech,gpio-vdd", 0); /* [P3592] for control vdd by gpio 141 , add by djian 20170123 pm*/

	if (data->vdd_ctrl<0) {
		pr_err("Regulator get failed vdd_ctrl");
		return -1;
	}
			...
			sx9310_platform_data_t *pdata)
{
	...
	kongkou_gpio = of_get_named_gpio_flags(dev->of_node,"Semtech,gpio-irq1", 0, NULL); 
	if(kongkou_gpio < 0)
		pr_debug("kongkou irq gpio is not available\n");

	return 0;
}

static ssize_t capsensor_read_RF_channel_status(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    char *ptr = page;
	...
	int err = 0;
        u8 status = 0;
	err = read_register(psx93XX_this,0x01,&status);
	status &= 0x06;  //indicates if proximity is currently being detected by CS1 or CS2
    	pr_debug("sx9310 rf channel status  = %u\n", status);
	ptr += sprintf(ptr,"%u\n",status);	
    }else{
        ptr += sprintf(ptr, "%u\n", 0);
    }
...
static ssize_t capsensor_config_check_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    char *ptr = page;
    u8 value = 0;
    int err = 0;
    ...
    if (sx9310_enable){
    	err = read_register(psx93XX_this, 0x10,&value);
    	if (!err && value == 0x57 ){    //check if register read error and register value is right
       		ptr += sprintf(ptr, "%u\n", 1);
    	}else{
    		ptr += sprintf(ptr, "%u\n", 0);
    	}
...
static ssize_t kongkou_status_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    char *ptr = page;
    int value;
    ...
    value = gpio_get_value(kongkou_gpio);
    printk("sx9310 %s: kongkou gpio value %d\n ",__func__,value);
    if(value)
	{
		//disable capsensor
		int  err ;
		if (sx9310_enable){
			...
			err = sx9310_power_ctl(psx93XX_this, false);     //modified by dingjian for power supply when system suspend 20170322
			if (err) {
				dev_err(psx93XX_this->pdev, "Failed to disable Capacitive Touch Controller power\n");
			}
		}
	}else{
		...
		if (!sx9310_enable){
			...
			err = sx9310_power_ctl(psx93XX_this, true);     //modified by dingjian for power supply when system suspend 20170322
			if (err) {
				dev_err(psx93XX_this->pdev, "Failed to disable Capacitive Touch Controller power\n");
			}
		}
	}
    pr_debug("Reading kongkuo status %d\n", value);
    ptr += sprintf(ptr, "%d\n", value);
...
static int sx9310_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!pplatData) {
  	...
  	if (gpio_is_valid(kongkou_gpio)) {
		/* sx9310 interrupt gpio request */
		err = gpio_request(kongkou_gpio, "KONGKOU_NIRQ");\
		if (err < 0) {
			dev_err(&client->dev, "gpio req failed for kongkou interrupt\n");
			kongkou_gpio = INVALID_GPIO;
			goto error_0;
		}
		err = gpio_direction_input(kongkou_gpio);
		if (err) {
			dev_err(&client->dev,"unable to set direction for gpio " \
					"[%d]\n", kongkou_gpio);
			goto error_0;
  	...
  	if (this) {
    	...
    	this->kongkou_irq = gpio_to_irq(kongkou_gpio);
		...
		PSX9310Device = pDevice;
      		...
      		__set_bit(KEY_KONGKOU_KEYCODE,input->keybit);
		...
		capsensor_proc = proc_create(PROC_CAPSENSOR_RF_STATUS,0666,NULL,&config_read_rf_channel_ops);
		if (capsensor_proc == NULL)
		{
			pr_info("create_proc_entry %s failed\n", PROC_CAPSENSOR_RF_STATUS);
		}
		else
		{
			pr_info("create proc entry %s success", PROC_CAPSENSOR_RF_STATUS);
		}
		capsensor_proc = proc_create(PROC_CAPSENSOR_CHECK_FILE,0666,NULL,&config_proc_capsensor_check_ops);
		if (capsensor_proc == NULL)
		{
			pr_info("create_proc_entry %s failed\n", PROC_CAPSENSOR_CHECK_FILE);
		}
		else
		{
			pr_info("create proc entry %s success", PROC_CAPSENSOR_CHECK_FILE);
		}
		kongkou_proc = proc_create(PROC_KONGKOU_STATUS, 0666, NULL, &config_kongkou_proc_ops);
		if (kongkou_proc == NULL)
		{
			pr_info("create_proc_entry %s failed\n", PROC_KONGKOU_STATUS);
		}
		else
		{
			pr_info("create proc entry %s success", PROC_KONGKOU_STATUS);
		}
